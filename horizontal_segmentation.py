...
def display_image(image, segments, name_image):
    ...
    for i in range(num_segments):
        y = i * (height // num_segments)
        ax.axhline(y=y, color='g', linestyle='-', linewidth=1)
        ax.text(width // 2, y + 2, str(i + 1), color='g', fontsize=10, horizontalalignment='center')

    ax.axhline(y=num_segments * (height // num_segments), color='g', linestyle='-', linewidth=1)
...
def calculate_absolute_vector(image, num_stripes):
    ...
    for i in range(num_stripes):
        segment = image[i * stripe_height:(i + 1) * stripe_height, :]
...
def main():
    ...
    for image_file in image_files:
        ...
        abs_vector = calculate_absolute_vector(image, num_stripes=5)
